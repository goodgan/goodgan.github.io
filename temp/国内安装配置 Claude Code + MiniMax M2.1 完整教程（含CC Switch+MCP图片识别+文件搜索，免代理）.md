# 国内安装配置 Claude Code + MiniMax M2.1 完整教程（免代理）
...
### （2）Python 3.10+（MCP 官方依赖）

MCP（图片识别、文件搜索）功能依赖 Python 环境，官方要求 3.10 及以上版本，国内可直装：

- **Windows/macOS**：下载地址：https://www.python.org/downloads/ （国内自动跳转镜像），安装时勾选“Add Python to PATH”，完成后验证。
...
- **验证安装**：
        `python --version  # 或 python3 --version，需输出 3.10.0+
pip --version     # 或 pip3 --version，需输出 22.0.0+
`

### （3）UV 包管理器（MCP 官方推荐，替代 pip，提速）

MiniMax 官方 MCP 文档推荐使用 UV 作为 Python 包管理器，安装速度更快，国内可直装：

```bash
# 全系统通用安装命令（国内镜像）
pip install uv --index-url https://pypi.tuna.tsinghua.edu.cn/simple
# 验证安装
uv --version  # 需输出 0.1.0+

```

### （4）MiniMax 账号与 Coding Plan API 密钥（关键，必做）
